  ...
  describe 'GET /api/v1/recipes' do
    context 'when country is provided' do
      it 'returns recipes for the given country' do
        ...
        expect(json_response.first[:id]).to eq(nil)
    ...
    #     allow_any_instance_of(RecipeService).to receive(:get_recipe).and_return({ hits: [{ 'recipe' => { 'label' => 'Recipe Title', 'image' => 'http://example.com/image.jpg', 'url' => 'http://example.com/recipe' } }] })
